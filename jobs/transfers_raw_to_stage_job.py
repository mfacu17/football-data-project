"""
ETL job to clean and load transfers from the 'raw' layer to the 'stage' layer in BigQuery.
...
Requirements:
    ...
    - Environment variables properly set:
        ...
        * CREDENTIALS_FILE
        * CONNECTOR_FILE
    - The 'raw.transfers_raw' table must exist in BigQuery.
    - GCS bucket configured to store rejected records.

Outputs:
    - Cleaned data is written to: `stage.transfers_stage`
    - Invalid rows are saved to GCS: `gs://<BUCKET_NAME>/errors/transfers_stage_errors/`
"""
...
def transfers_raw_to_stage():
    """
    Extracts, validates, and loads 'transfers' data from the raw layer to the stage layer in BigQuery.

    Steps:
        1. Create a Spark session with the necessary GCP configurations.
        2. Read the 'transfers_raw' table from the 'raw' dataset in BigQuery.
        ...
        5. Fill missing values in selected non-critical columns with default values:
            - 'transfer_fee', 'market_value_in_eur' → -1
        6. Drop non-essential columns 'transfer_season', 'from_club_name', 'to_club_name' and 'player_name'
    ...
    df_nulls = df.filter(
        ...
        col("from_club_name").isNull() |
        col("to_club_name").isNull() 
    )
    ...
    if null_count > 0:
        df_nulls.write.mode("overwrite").csv(
            f"gs://{BUCKET_NAME}/errors/transfers_stage_errors",
            header=True
        )
    
    # Eliminate nulls and duplicates
    df_clean = df.dropna(subset=["player_id", "transfer_date", "from_club_name", "to_club_name"]) \
                .fillna({
                    "transfer_fee":-1,
                    "market_value_in_eur":-1
                })

    # Drop 'transfer_season', 'from_club_name', 'to_club_name' and 'player_name'
    df_clean = df_clean.drop("transfer_season", "from_club_id", "to_club_id", "player_name")
    ...
    print("✅ transfers stage table loaded successfully.")
